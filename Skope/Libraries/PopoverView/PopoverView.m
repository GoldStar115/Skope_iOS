...
@property (nonatomic, readwrite) PopoverMaskType maskType;
@property (nonatomic, strong) PopoverOverlayView *overlayView;
...
+ (PopoverView *)showPopoverAtPoint:(CGPoint)point inView:(UIView *)view maskType:(PopoverMaskType)hudMaskType withText:(NSString *)text delegate:(id<PopoverViewDelegate>)delegate {
    PopoverView *popoverView = [[PopoverView alloc] initWithFrame:CGRectZero];
    popoverView.overlayView = [PopoverOverlayView ShowWithMaskType:hudMaskType];
    [popoverView showAtPoint:point inView:view maskType:(PopoverMaskType)hudMaskType withText:text];
    popoverView.delegate = delegate;
    [popoverView RELEASE];
    return popoverView;
}

+ (PopoverView *)showPopoverAtPoint:(CGPoint)point inView:(UIView *)view maskType:(PopoverMaskType)hudMaskType withTitle:(NSString *)title withText:(NSString *)text delegate:(id<PopoverViewDelegate>)delegate {
    PopoverView *popoverView = [[PopoverView alloc] initWithFrame:CGRectZero];
    popoverView.overlayView = [PopoverOverlayView ShowWithMaskType:hudMaskType];
    [popoverView showAtPoint:point inView:view maskType:(PopoverMaskType)hudMaskType withTitle:title withText:text];
    popoverView.delegate = delegate;
    [popoverView RELEASE];
    return popoverView;
}

+ (PopoverView *)showPopoverAtPoint:(CGPoint)point inView:(UIView *)view maskType:(PopoverMaskType)hudMaskType withViewArray:(NSArray *)viewArray delegate:(id<PopoverViewDelegate>)delegate {
    PopoverView *popoverView = [[PopoverView alloc] initWithFrame:CGRectZero];
    popoverView.overlayView = [PopoverOverlayView ShowWithMaskType:hudMaskType];
    [popoverView showAtPoint:point inView:view maskType:(PopoverMaskType)hudMaskType withViewArray:viewArray];
    popoverView.delegate = delegate;
    [popoverView RELEASE];
    return popoverView;
}

+ (PopoverView *)showPopoverAtPoint:(CGPoint)point inView:(UIView *)view maskType:(PopoverMaskType)hudMaskType withTitle:(NSString *)title withViewArray:(NSArray *)viewArray delegate:(id<PopoverViewDelegate>)delegate {
    PopoverView *popoverView = [[PopoverView alloc] initWithFrame:CGRectZero];
    popoverView.overlayView = [PopoverOverlayView ShowWithMaskType:hudMaskType];
    [popoverView showAtPoint:point inView:view maskType:(PopoverMaskType)hudMaskType withTitle:title withViewArray:viewArray];
    popoverView.delegate = delegate;
    [popoverView RELEASE];
    return popoverView;
}

+ (PopoverView *)showPopoverAtPoint:(CGPoint)point inView:(UIView *)view maskType:(PopoverMaskType)hudMaskType withStringArray:(NSArray *)stringArray delegate:(id<PopoverViewDelegate>)delegate {
    PopoverView *popoverView = [[PopoverView alloc] initWithFrame:CGRectZero];
    popoverView.overlayView = [PopoverOverlayView ShowWithMaskType:hudMaskType];
    [popoverView showAtPoint:point inView:view maskType:(PopoverMaskType)hudMaskType withStringArray:stringArray];
    popoverView.delegate = delegate;
    [popoverView RELEASE];
    return popoverView;
}

+ (PopoverView *)showPopoverAtPoint:(CGPoint)point inView:(UIView *)view maskType:(PopoverMaskType)hudMaskType withTitle:(NSString *)title withStringArray:(NSArray *)stringArray delegate:(id<PopoverViewDelegate>)delegate {
    PopoverView *popoverView = [[PopoverView alloc] initWithFrame:CGRectZero];
    popoverView.overlayView = [PopoverOverlayView ShowWithMaskType:hudMaskType];
    [popoverView showAtPoint:point inView:view maskType:(PopoverMaskType)hudMaskType withTitle:title withStringArray:stringArray];
    popoverView.delegate = delegate;
    [popoverView RELEASE];
    return popoverView;
}

+ (PopoverView *)showPopoverAtPoint:(CGPoint)point inView:(UIView *)view maskType:(PopoverMaskType)hudMaskType withStringArray:(NSArray *)stringArray withImageArray:(NSArray *)imageArray delegate:(id<PopoverViewDelegate>)delegate {
    PopoverView *popoverView = [[PopoverView alloc] initWithFrame:CGRectZero];
    popoverView.overlayView = [PopoverOverlayView ShowWithMaskType:hudMaskType];
    [popoverView showAtPoint:point inView:view maskType:(PopoverMaskType)hudMaskType withStringArray:stringArray withImageArray:imageArray];
    popoverView.delegate = delegate;
    [popoverView RELEASE];
    return popoverView;
}

+ (PopoverView *)showPopoverAtPoint:(CGPoint)point inView:(UIView *)view maskType:(PopoverMaskType)hudMaskType withTitle:(NSString *)title withStringArray:(NSArray *)stringArray withImageArray:(NSArray *)imageArray delegate:(id<PopoverViewDelegate>)delegate {
    PopoverView *popoverView = [[PopoverView alloc] initWithFrame:CGRectZero];
    popoverView.overlayView = [PopoverOverlayView ShowWithMaskType:hudMaskType];
    [popoverView showAtPoint:point inView:view maskType:(PopoverMaskType)hudMaskType withTitle:title withStringArray:stringArray withImageArray:imageArray];
    popoverView.delegate = delegate;
    [popoverView RELEASE];
    return popoverView;
}

+ (PopoverView *)showPopoverAtPoint:(CGPoint)point inView:(UIView *)view maskType:(PopoverMaskType)hudMaskType withTitle:(NSString *)title withContentView:(UIView *)cView delegate:(id<PopoverViewDelegate>)delegate {
    PopoverView *popoverView = [[PopoverView alloc] initWithFrame:CGRectZero];
    popoverView.overlayView = [PopoverOverlayView ShowWithMaskType:hudMaskType];
    [popoverView showAtPoint:point inView:view maskType:(PopoverMaskType)hudMaskType withTitle:title withContentView:cView];
    popoverView.delegate = delegate;
    [popoverView RELEASE];
    return popoverView;
}

+ (PopoverView *)showPopoverAtPoint:(CGPoint)point inView:(UIView *)view maskType:(PopoverMaskType)hudMaskType withContentView:(UIView *)cView delegate:(id<PopoverViewDelegate>)delegate {
    PopoverView *popoverView = [[PopoverView alloc] initWithFrame:CGRectZero];
    popoverView.overlayView = [PopoverOverlayView ShowWithMaskType:hudMaskType];
    [popoverView showAtPoint:point inView:view maskType:(PopoverMaskType)hudMaskType withContentView:cView];
    popoverView.delegate = delegate;
    [popoverView RELEASE];
    return popoverView;
}
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view maskType:(PopoverMaskType)hudMaskType withText:(NSString *)text
{
    ...
    [self showAtPoint:point inView:view maskType:(PopoverMaskType)hudMaskType withViewArray:[NSArray arrayWithObject:textView]];
}

- (void)showAtPoint:(CGPoint)point inView:(UIView *)view maskType:(PopoverMaskType)hudMaskType withTitle:(NSString *)title withText:(NSString *)text
{
    UIFont *font = kTextFont;
    ...
    [self showAtPoint:point inView:view maskType:(PopoverMaskType)hudMaskType withTitle:title withViewArray:[NSArray arrayWithObject:textView]];
}

- (void)showAtPoint:(CGPoint)point inView:(UIView *)view maskType:(PopoverMaskType)hudMaskType withViewArray:(NSArray *)viewArray
{
    ...
    [self showAtPoint:point inView:view maskType:(PopoverMaskType)hudMaskType withContentView:container];
}

- (void)showAtPoint:(CGPoint)point inView:(UIView *)view maskType:(PopoverMaskType)hudMaskType withTitle:(NSString *)title withViewArray:(NSArray *)viewArray
{
    UIView *container = [[UIView alloc] initWithFrame:CGRectZero];
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view maskType:(PopoverMaskType)hudMaskType withStringArray:(NSArray *)stringArray
{
    ...
    [self showAtPoint:point inView:view maskType:(PopoverMaskType)hudMaskType withViewArray:labelArray];
}

- (void)showAtPoint:(CGPoint)point inView:(UIView *)view maskType:(PopoverMaskType)hudMaskType withTitle:(NSString *)title withStringArray:(NSArray *)stringArray
 {
    NSMutableArray *labelArray = [[NSMutableArray alloc] initWithCapacity:stringArray.count];
    ...
    [self showAtPoint:point inView:view maskType:(PopoverMaskType)hudMaskType withTitle:title withViewArray:labelArray];
}

- (void)showAtPoint:(CGPoint)point inView:(UIView *)view maskType:(PopoverMaskType)hudMaskType withStringArray:(NSArray *)stringArray withImageArray:(NSArray *)imageArray
{
    ...
    [self showAtPoint:point inView:view maskType:(PopoverMaskType)hudMaskType withViewArray:tempViewArray];
}

- (void)showAtPoint:(CGPoint)point inView:(UIView *)view maskType:(PopoverMaskType)hudMaskType withTitle:(NSString *)title withStringArray:(NSArray *)stringArray withImageArray:(NSArray *)imageArray
{
    NSAssert((stringArray.count == imageArray.count), @"stringArray.count should equal imageArray.count");
    NSMutableArray* tempViewArray = [self makeTempViewsWithStrings:stringArray andImages:imageArray];
        
    [self showAtPoint:point inView:view maskType:(PopoverMaskType)hudMaskType withTitle:title withViewArray:tempViewArray];
}
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view maskType:(PopoverMaskType)hudMaskType withTitle:(NSString *)title withContentView:(UIView *)cView
{
    [self showAtPoint:point inView:view maskType:(PopoverMaskType)hudMaskType withTitle:title withViewArray:[NSArray arrayWithObject:cView]];
}

- (void)showAtPoint:(CGPoint)point inView:(UIView *)view maskType:(PopoverMaskType)hudMaskType withContentView:(UIView *)cView {
    
    //self.overlayView = [PopoverOverlayView ShowWithMaskType:hudMaskType];
...
{
    [self dismiss:YES completion:completion];
}

- (void)dismiss:(BOOL)animated completion:(dispatch_block_t)completion
{
    if (!animated)
    {
...
@implementation PopoverOverlayView

+ (PopoverOverlayView*)ShowWithMaskType:(PopoverMaskType)maskType {
    
    PopoverOverlayView* sharedView = [[self alloc] initWithFrame:[UIApplication sharedApplication].keyWindow.bounds];
    ...
    //UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(overlayViewDidReceiveTouchEvent:forEvent:)];
