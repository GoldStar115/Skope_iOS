...
// Add by Nguyen Truong Luu for Rotate bubble image chat item
...
- (UIImage *)jsq_horizontallyFlippedImageFromImage:(UIImage *)image
{
    ...
    // Flip horizontally
    UIImage *horizontallyFlippedImage = [UIImage imageWithCGImage:image.CGImage
                                                            scale:image.scale
                                                      orientation:UIImageOrientationUpMirrored];
    
    // Then flip vertical => final image for Incoming message bubble
    return [horizontallyFlippedImage verticalFlip];
}
