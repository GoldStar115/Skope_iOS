...
#define kCellContentTopPadding          5
#define kCellContentLeftPadding         20
#define kCellContentVerticalSpacing     10
...
- (BOOL)textView:(UITextView *)textView shouldInteractWithURL:(NSURL *)URL inRange:(NSRange)characterRange NS_AVAILABLE_IOS(7_0) {
    ...
    if ([_delegate respondsToSelector:@selector(PostCell:wantToOpenURL:)]) {
        [_delegate PostCell:self wantToOpenURL:URL];
    }
...
- (void)configureCellDisplayWithPostInfo:(NSMutableDictionary*)postInfo tableViewWidth:(CGFloat)tableViewWidth indexPath:(NSIndexPath*)indexPath nodeConstructionQueue:(NSOperationQueue*)nodeConstructionQueue {
    ...
    if (strongSelf) {
        ...
        if ([postInfo[@"user"][@"id"] isEqualToString:[UserDefault currentUser].u_id] || _isOnMyProfileCell) {
        ...
        if (next_to_distance && next_to_distance.length > 0) {
            ...
            //  Config frame for label after change text
            
            NSString *text = newLocationString;
            CGFloat width = strongSelf.lbl_postedAway.bounds.size.width;
            UIFont *font = strongSelf.lbl_postedAway.font;
            ...
            CGRect newFrame = CGRectMake(CGRectGetMinX(strongSelf.lbl_postedAway.frame), CGRectGetMinY(strongSelf.lbl_postedAway.frame), CGRectGetWidth(strongSelf.lbl_postedAway.frame), size.height);
            [strongSelf.lbl_postedAway setFrame:newFrame];
...
- (void)setContentText:(NSString *)contentText {
    ...
    NSString *template = [ProfilePostCell htmlTemplate];
