...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	//---------------------------------------------------------------------------------------------------------------------------------------------
	CreateMessageItem(user1, groupId, user2);
	CreateMessageItem(user2, groupId, user1);
	//---------------------------------------------------------------------------------------------------------------------------------------------
	return groupId;
}
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
    ...
    for (PFUser *user in users)
	{
        // TODO: need to create a new method for this
...
//    [query whereKey:PF_MESSAGES_LASTUSER equalTo:endUser];
	[query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error)
	{
		if (error == nil)
		{
			if ([objects count] == 0)
			{
				...
				message[PF_MESSAGES_DESCRIPTION] = endUser[PF_USER_FULLNAME];
				message[PF_MESSAGES_LASTUSER] = endUser;
                message[PF_MESSAGES_LASTUSER_EMAIL] = endUser[PF_USER_EMAILCOPY];
				message[PF_MESSAGES_LASTMESSAGE] = @"";
                message[PF_MESSAGES_STATUS] = @"hide";
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
    NSLog(@"DeleteMessageItem %@",message);
    message[PF_MESSAGES_STATUS] = @"hide";
	[message saveInBackgroundWithBlock:^(BOOL succeeded, NSError *error)
	{
	...
	[query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error)
	{
		if (error == nil)
		{
			for (PFObject *message in objects)
			{
                ...
                message[PF_MESSAGES_STATUS] = @"show";
